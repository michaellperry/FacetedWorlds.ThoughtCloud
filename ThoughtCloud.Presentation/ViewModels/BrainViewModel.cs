using UpdateControls.Fields;

namespace ThoughtCloud_Presentation.ViewModels
{
    public class BrainViewModel : IPresentationViewModel
    {
        public enum StateId
        {
            Start,
            Correspondence,
            Logically,
            Physically,
            Device,
            Workstation,
            Service,
            Need,
            End
        }

        private Independent<StateId> _state = new Independent<StateId>();

        public StateId State
        {
            get { return _state; }
        }

        public bool Forward()
        {
            if (_state.Value != StateId.End - 1)
            {
                _state.Value = _state.Value + 1;
                return true;
            }
            else
                return false;
        }

        public bool Backward()
        {
            if (_state.Value != StateId.Start)
            {
                _state.Value = _state.Value - 1;
                return true;
            }
            else
                return false;
        }

        public string Title
        {
            get
            {
                switch (_state.Value)
                {
                    case StateId.Start:
                        return "This is your data";
                    case StateId.Correspondence:
                        return "This is your data in Correspondence";
                    case StateId.Logically:
                        return "Logically, in one community brain";
                    case StateId.Physically:
                        return "Physically, distributed ...";
                    case StateId.Device:
                        return "... to any device ...";
                    case StateId.Workstation:
                        return "... workstation ...";
                    case StateId.Service:
                        return "... and service ...";
                    case StateId.Need:
                        return "... that needs it";
                }
                return string.Empty;
            }
        }
    }
}
